...
function Subscribers({ navigation }) {
    ...
    useEffect(() => {
        const fetchSubscriptions = async () => {
            try {
                const response = await userApi.getTierMembers(user.id, 0, 1); 
                setMembers(response.data.members);
                setHasSubscriptions(response.data.members.length > 0); // Check for subscriptions
            } catch (error) {
